...
class FindLiteHandlerTest extends UnitTestCase
{
     ...
     *
     * @dataProvider \Backend\Api\RpcSkeleton\Tests\Unit\DataProvider\Domain\Query\FindLiteHandlerDataProvider::getDataForHandleMethod()
     *
    ...
    public function handleShouldProcessCommandAndReturnArrayTest(array $mockArgs, array $mockTimes): void
    {
        ...
        foreach ($mockArgs['handle'] as $i => $domainEntityItemEntityMock) {
            $handles[] = $this->createDomainEntityItemEntityMock($domainEntityItemEntityMock, $mockTimes['handle'][$i]);
        }
        ...
        self::assertEquals($handles[0]->getId()->toNative(), $result[0]->getId()->toNative());
        self::assertEquals($handles[0]->getName()->toNative(), $result[0]->getName()->toNative());
        self::assertEquals($handles[0]->getUrl()->toNative(), $result[0]->getUrl()->toNative());
